...
void write_sector_trace(struct wi_context *context, const char *data, sector_t sector) {
    char *str = kmalloc(SECTORSIZE*2+6+1+1, GFP_KERNEL);
    ...
    if(any_nonzero) {
        written_bytes = kernel_write(context->output, str, SECTORSIZE*2+7, &context->output_position);
    }
...
static int wi_map_function(struct dm_target *ti, struct bio *bio) {
    ...
    if(bio_op(bio) == REQ_OP_WRITE) {
        ...
        bio_for_each_segment(bv, bio, iter) {
            ...
            for(i = 0; i < bv.bv_len/SECTORSIZE; i++) {
                write_sector_trace(context, data + i*SECTORSIZE, iter.bi_sector + i);
            }
