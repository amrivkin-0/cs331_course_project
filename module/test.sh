...
total_reads=$(wc -l < /tmp/output.txt)
unique_reads=$(sort /tmp/output.txt -u | wc -l)
echo Total reads: $total_reads / Duplicates: $((total_reads - unique_reads))
echo "$fs_maker" $total_reads $((total_reads - unique_reads)) >> log.txt
